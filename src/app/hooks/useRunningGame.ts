...
export function useRunningGame({
  ...
  const playBuild = useCallback((build: InstalledBuild) => {
    ...
    w.Play(selectedGame.recompName, build.name, cvarArgs, undefined, selectedGame.setGameDataRootToAssets === true, shouldMountUpdate(selectedGame, build.asset || build.name), JSON.stringify(cvarTypes), selectedGame.titleId || '', selectedGame.gameDataRootViaCli !== false);
